...
exports.parameters = (
  ...
  const packageFailText = (score, package, failures) =>
    `<p>The page <strong>did not pass</strong> the <code>${package}</code> test and received a score of ${score} on <code>${package}</code>. There was at least one failure of:</p>${joiner}<ul>${innerJoiner}${failures}${joiner}</ul>`;
  ...
  const customFailText = (score, test) =>
    `<p>The page <strong>did not pass</strong> the <code>${test}</code> test and received a score of ${score} on <code>${test}</code>.</p>`;
  ...
  if (deficit.aatt) {
    ...
    paramData.aattResult = packageFailText(deficit.aatt, 'aatt', aattFailures);
  }
  else if (scoreData.inferences.aatt) {
    paramData.aattResult = testCrashText(deficitData.aatt, 'aatt');
  }
  else {
    paramData.aattResult = packageSucceedText('aatt');
  }
  // axe
  if (deficit.axe) {
