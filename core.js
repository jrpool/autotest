...
const scriptHandler = async (
  what, strict, acts, stage, hostIndex, server
) => {
  ...
  const report = {};
  report.script = server.query.scriptName;
  report.batch = server.query.batchName;
  report.what = what;
  report.strict = strict;
  report.testDate = new Date().toISOString().slice(0, 10);
  report.timeStamp = server.query.timeStamp;
  report.logCount = 0;
  report.logSize = 0;
  ...
  const reportSuffix = `${server.query.timeStamp}${hostSuffix}`;
  // Perform the specified acts and add the results and exhibits to the report.
  await doActs(acts, report, 0, null, reportSuffix, server.query.reportDir);
  // Close the browser.
  await closeBrowser();
  ...
  if (report.exhibits) {
    server.query.exhibits = report.exhibits;
    delete report.exhibits;
  }
  // Otherwise, i.e. if no exhibits have been added to the report:
  else {
    // Add properties to the query.
    server.query.exhibits = '<p><strong>None</strong></p>';
    server.query.hostIndex = hostIndex;
  }
  // Convert the report to JSON and add it to the query.
  const reportJSON = JSON.stringify(report, null, 2);
  server.query.report = reportJSON.replace(/</g, '&lt;');
  // Update the report file.
  await reportFileUpdate(server.query.reportDir, reportSuffix, reportJSON, true);
  // Render and serve the output.
  server.render('', stage, 'out', server.query, server.response);
  return report;
};
